...
function buildRadarOption(blocks: ReportBlocks) {
...
const Assessment: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        <Spin size="large" />
        <p className="text-gray-500">正在加载评估报告...</p>
      </div>
    );
  }
  ...
  return (
    <div ref={mainRef} className="animate-fade-in">
      {/* 锚点导航 */}
      <div className="sticky top-0 z-10 bg-white border-b border-gray-100 -mx-6 px-6 py-2 mb-6 flex gap-2 overflow-x-auto">
        {ANCHORS.map(({ id, label }) => (
          <button
            key={id}
            onClick={() => scrollTo(id)}
            className="text-xs text-gray-500 hover:text-blue-600 hover:bg-blue-50 px-2 py-1 rounded whitespace-nowrap transition-colors"
          >
            {label}
          </button>
        ))}
      ...
      {/* Block: header */}
      <section id="block-header" className="mb-6 bg-gray-50 rounded-xl p-4 border border-gray-100">
        <p className="text-sm text-gray-500 mb-2">
          评估 ID：<span className="font-mono text-gray-700">{blocks.header?.assessment_id}</span>
        </p>
      ...
      {/* Block: radar */}
      <section id="block-radar" className="mb-6 bg-white rounded-xl border border-gray-100 shadow-sm p-5">
        <h3 className="font-bold text-gray-800 text-lg mb-4">六维能力雷达图</h3>
        <div className="flex flex-col md:flex-row gap-6 items-start">
          <div className="flex-1 min-h-[300px]">
            <ReactECharts
              option={buildRadarOption(blocks)}
              style={{ height: 300 }}
            />
          </div>
      ...
      {/* Block: overview */}
      {blocks.overview && (
        <section id="block-overview" className="mb-6 bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl border border-blue-100 p-6">
          <h2 className="text-2xl font-bold text-gray-900 mb-1">
            {blocks.overview.persona_label || '综合画像'}
          </h2>
          {blocks.overview.narrative_intro && (
            <p className="text-gray-600 text-sm leading-relaxed mb-4">{blocks.overview.narrative_intro}</p>
          )}
          {(blocks.overview.top_cards || []).length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
              {(blocks.overview.top_cards || []).map((card, i) => (
                <div key={i} className="bg-white rounded-lg p-4 shadow-sm border border-white">
                  <p className="font-semibold text-gray-800 text-sm">{card.title || (card as unknown as string)}</p>
                  {card.description && (
      ...
      {/* Block: action */}
      {blocks.action && (
        <section id="block-action" className="mb-6 bg-white rounded-xl border border-gray-100 shadow-sm p-5">
          <h3 className="font-bold text-gray-800 text-lg mb-4">行动建议</h3>
      ...
      {blocks.unlock && (blocks.unlock.items || []).length > 0 && (
        <section id="block-unlock" className="mb-6 bg-gray-50 rounded-xl border border-gray-200 p-5">
          <h3 className="font-bold text-gray-600 text-lg mb-3">解锁更多分析</h3>
